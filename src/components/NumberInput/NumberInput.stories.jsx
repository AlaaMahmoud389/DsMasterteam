import React, { useState } from 'react';
import { InputPrefixSuffix, NumberInput } from './NumberInput';

export default {
  title: 'Components/NumberInput',
  parameters: { layout: 'padded' },
};

/* ── Playground — NumberInput ───────────────────────────────── */
export const Playground = {
  name: 'Playground — Number Input',
  argTypes: {
    state:          { control: 'radio', options: ['default', 'hovered', 'pressed', 'focused', 'disabled', 'read-only'] },
    inputStyle:     { control: 'radio', options: ['default', 'filled-lighter', 'filled-darker'] },
    btnStyle:       { control: 'radio', options: ['solid', 'subtle'] },
    size:           { control: 'radio', options: ['large', 'medium'] },
    labelType:      { control: 'radio', options: ['regular', 'semibold'] },
    error:          { control: 'boolean' },
    showLabel:      { control: 'boolean' },
    showHelperText: { control: 'boolean' },
    required:       { control: 'boolean' },
    rtl:            { control: 'boolean' },
    label:          { control: 'text' },
    helperText:     { control: 'text' },
  },
  args: {
    state: 'default',
    inputStyle: 'default',
    btnStyle: 'solid',
    size: 'large',
    labelType: 'regular',
    error: false,
    showLabel: true,
    showHelperText: true,
    required: false,
    rtl: false,
    label: 'Label',
    helperText: 'Help Text',
  },
  render: (args) => {
    const [value, setValue] = useState(0);
    return (
      <div style={{ fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif", padding: 16, maxWidth: 280 }}>
        <NumberInput {...args} value={value} onChange={setValue} />
      </div>
    );
  },
};

/* ── Playground — Input Prefix-Suffix ───────────────────────── */
export const PlaygroundButton = {
  name: 'Playground — Input Prefix-Suffix',
  argTypes: {
    type:     { control: 'radio', options: ['plus', 'minus'] },
    state:    { control: 'radio', options: ['default', 'hovered', 'pressed', 'focused', 'selected', 'disabled'] },
    btnStyle: { control: 'radio', options: ['solid', 'subtle'] },
    size:     { control: 'radio', options: ['large', 'medium'] },
  },
  args: { type: 'plus', state: 'default', btnStyle: 'solid', size: 'large' },
  render: (args) => (
    <div style={{ fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif", padding: 16 }}>
      <InputPrefixSuffix {...args} />
    </div>
  ),
};

/* ── All States ─────────────────────────────────────────────── */
export const AllStates = {
  name: 'Number Input — All States',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      {['default', 'hovered', 'pressed', 'focused', 'disabled', 'read-only'].map((state) => (
        <div key={state}>
          <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>{state}</p>
          <NumberInput
            label="Label"
            showLabel
            helperText="Help Text"
            showHelperText
            state={state}
            value={state === 'read-only' ? 42 : 0}
            placeholder="00000"
          />
        </div>
      ))}
    </div>
  ),
};

/* ── Error States ───────────────────────────────────────────── */
export const ErrorStates = {
  name: 'Number Input — Error States',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      {['default', 'hovered', 'focused', 'disabled'].map((state) => (
        <div key={state}>
          <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>{state} + error</p>
          <NumberInput
            label="Label"
            showLabel
            helperText="Help Text"
            showHelperText
            state={state}
            error
            value={0}
          />
        </div>
      ))}
    </div>
  ),
};

/* ── Sizes ──────────────────────────────────────────────────── */
export const Sizes = {
  name: 'Number Input — Sizes',
  render: () => (
    <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap', fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif", alignItems: 'flex-end' }}>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Large</p>
        <NumberInput label="Label" showLabel size="large" value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Medium</p>
        <NumberInput label="Label" showLabel size="medium" value={0} />
      </div>
    </div>
  ),
};

/* ── Input Styles ───────────────────────────────────────────── */
export const InputStyles = {
  name: 'Number Input — Input Styles',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      {[
        { inputStyle: 'default',        label: 'Style = Default' },
        { inputStyle: 'filled-lighter', label: 'Style = Filled Lighter' },
        { inputStyle: 'filled-darker',  label: 'Style = Filled Darker' },
      ].map(({ inputStyle, label }) => (
        <div key={inputStyle}>
          <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>{label}</p>
          <NumberInput
            label="Label"
            showLabel
            helperText="Help Text"
            showHelperText
            inputStyle={inputStyle}
            value={42}
          />
        </div>
      ))}
    </div>
  ),
};

/* ── Button Styles ──────────────────────────────────────────── */
export const ButtonStyles = {
  name: 'Number Input — Button Styles',
  render: () => (
    <div style={{ display: 'flex', gap: 40, flexWrap: 'wrap', fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Solid buttons</p>
        <NumberInput label="Label" showLabel btnStyle="solid" value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Subtle buttons</p>
        <NumberInput label="Label" showLabel btnStyle="subtle" value={0} />
      </div>
    </div>
  ),
};

/* ── Label Variants ─────────────────────────────────────────── */
export const LabelVariants = {
  name: 'Number Input — Label Variants',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Show Label = Off</p>
        <NumberInput showLabel={false} value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Regular label</p>
        <NumberInput label="Label" showLabel labelType="regular" value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Semibold label</p>
        <NumberInput label="Label" showLabel labelType="semibold" value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Required</p>
        <NumberInput label="Label" showLabel required value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Disabled label</p>
        <NumberInput label="Label" showLabel state="disabled" value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Large label size</p>
        <NumberInput label="Label" showLabel size="large" value={0} />
      </div>
    </div>
  ),
};

/* ── Helper Text ────────────────────────────────────────────── */
export const HelperTextVariants = {
  name: 'Number Input — Helper Text',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>No helper text</p>
        <NumberInput label="Label" showLabel showHelperText={false} value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>With helper text</p>
        <NumberInput label="Label" showLabel helperText="Help Text" showHelperText value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Helper + Error</p>
        <NumberInput label="Label" showLabel helperText="Help Text" showHelperText error value={0} />
      </div>
    </div>
  ),
};

/* ── RTL ────────────────────────────────────────────────────── */
export const RTLNumberInput = {
  name: 'Number Input — RTL',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>RTL — Default</p>
        <NumberInput label="تسمية" showLabel helperText="نص مساعد" showHelperText rtl value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>RTL — Error</p>
        <NumberInput label="تسمية" showLabel helperText="نص مساعد" showHelperText error rtl value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>RTL — Disabled</p>
        <NumberInput label="تسمية" showLabel state="disabled" rtl value={0} />
      </div>
      <div>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>RTL — Filled darker</p>
        <NumberInput label="تسمية" showLabel inputStyle="filled-darker" rtl required value={42} />
      </div>
    </div>
  ),
};

/* ── Input Prefix-Suffix — All States ───────────────────────── */
export const PrefixSuffixAllStates = {
  name: 'Input Prefix-Suffix — All States',
  render: () => (
    <div style={{ display: 'flex', gap: 48, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif", flexWrap: 'wrap', alignItems: 'flex-start' }}>
      {[
        { type: 'plus',  btnStyle: 'solid',  label: 'Solid — Plus' },
        { type: 'minus', btnStyle: 'solid',  label: 'Solid — Minus' },
        { type: 'plus',  btnStyle: 'subtle', label: 'Subtle — Plus' },
        { type: 'minus', btnStyle: 'subtle', label: 'Subtle — Minus' },
      ].map(({ type, btnStyle, label }) => (
        <div key={`${type}-${btnStyle}`}>
          <p style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 600, color: '#000b36' }}>{label}</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {['default', 'hovered', 'pressed', 'selected', 'focused', 'disabled'].map((state) => (
              <div key={state} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#6C7C96', width: 72, flexShrink: 0 }}>{state}</span>
                <InputPrefixSuffix type={type} state={state} btnStyle={btnStyle} size="large" />
              </div>
            ))}
          </div>
        </div>
      ))}
    </div>
  ),
};

/* ── Input Prefix-Suffix — Sizes ────────────────────────────── */
export const PrefixSuffixSizes = {
  name: 'Input Prefix-Suffix — Sizes',
  render: () => (
    <div style={{ display: 'flex', gap: 40, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif", alignItems: 'flex-end', flexWrap: 'wrap' }}>
      {['solid', 'subtle'].map((btnStyle) =>
        ['plus', 'minus'].map((type) =>
          ['large', 'medium'].map((size) => (
            <div key={`${btnStyle}-${type}-${size}`}>
              <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>{btnStyle} {type} {size}</p>
              <InputPrefixSuffix type={type} btnStyle={btnStyle} size={size} />
            </div>
          ))
        )
      )}
    </div>
  ),
};

/* ── Interactive demo ───────────────────────────────────────── */
export const InteractiveDemo = {
  name: 'Number Input — Interactive',
  render: () => {
    const [val, setVal] = useState(0);
    return (
      <div style={{ fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif", padding: 16 }}>
        <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>Click +/− to change value</p>
        <NumberInput
          label="Quantity"
          showLabel
          helperText="Enter a number between 0 and 100"
          showHelperText
          value={val}
          onChange={setVal}
          min={0}
          max={100}
          step={1}
          required
        />
        <p style={{ marginTop: 12, fontSize: 14, color: '#6c7c96' }}>
          Current value: <strong>{val}</strong>
        </p>
      </div>
    );
  },
};

/* ── Full Matrix ────────────────────────────────────────────── */
export const FullMatrix = {
  name: 'Number Input — Full Matrix',
  render: () => (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 32, fontFamily: "'IBM Plex Sans Arabic', system-ui, sans-serif" }}>
      {['default', 'hovered', 'focused', 'pressed', 'disabled', 'read-only'].map((state) =>
        [false, true].map((error) => (
          <div key={`${state}-${error}`}>
            <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>{state}{error ? ' + error' : ''}</p>
            <NumberInput
              label="Label"
              showLabel
              helperText="Help Text"
              showHelperText
              state={state}
              error={error}
              value={state === 'read-only' ? 42 : 0}
              size="large"
            />
          </div>
        ))
      )}
      {['default', 'focused', 'disabled'].map((state) => (
        <div key={`medium-${state}`}>
          <p style={{ margin: '0 0 8px', fontSize: 12, fontWeight: 600, color: '#6C7C96' }}>medium {state}</p>
          <NumberInput
            label="Label"
            showLabel
            helperText="Help Text"
            showHelperText
            state={state}
            value={0}
            size="medium"
          />
        </div>
      ))}
    </div>
  ),
};
